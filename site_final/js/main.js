...
/* ──────────────────────────────────────────────────────────────
   11. СОХРАНЕНИЕ ПОЗИЦИИ СКРОЛЛА ЛЕВОГО МЕНЮ
   При переходе между страницами меню остаётся на том же месте
────────────────────────────────────────────────────────────── */
function initMenuScrollMemory() {
    ...
    const savedScroll = sessionStorage.getItem('menuScrollTop');
    if (savedScroll !== null) {
        menu.scrollTop = parseInt(savedScroll, 10);
    }

    // Сохраняем позицию скролла при клике на любую ссылку меню
    menu.addEventListener('click', (e) => {
        if (e.target.tagName === 'A') {
            sessionStorage.setItem('menuScrollTop', menu.scrollTop);
        }
    });
}
